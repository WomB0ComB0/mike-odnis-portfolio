document.addEventListener('DOMContentLoaded', () => {
  ...
  if (projectsContainer) {
    projectsContainer.innerHTML = portfolioData.projects.map(p => `
      <div class="bg-zinc-900 rounded-lg p-6 hover:-translate-y-2 transition-transform duration-300 border border-zinc-800 hover:border-emerald-400/50 flex flex-col h-full fade-in">
        <div class="flex justify-between items-start mb-4">
          ...
          <div class="flex space-x-4">
            <a href="${p.github}" class="text-zinc-400 hover:text-emerald-400 transition-colors"><i data-lucide="github" class="w-5 h-5"></i></a>
            <a href="${p.link}" class="text-zinc-400 hover:text-emerald-400 transition-colors"><i data-lucide="external-link" class="w-5 h-5"></i></a>
          </div>
        </div>
        <h3 class="text-xl font-bold text-zinc-100 mb-3 group-hover:text-emerald-400 transition-colors">${p.title}</h3>
        <p class="text-zinc-400 text-sm mb-6 flex-grow">${p.description}</p>
        <ul class="flex flex-wrap gap-3 mt-auto">
          ${p.tags.map(t => `<li class="text-xs font-mono text-zinc-500">${t}</li>`).join('')}
        </ul>
      </div>
    `).join('');
  }
